...
\end{tcolorbox}
\subsection{Passivity Property of the System}

The total energy of the Triangle system, which consists of the pendulum body and the wheel, is the sum of its kinetic and potential energies:
\begin{equation}
E = T + V
\end{equation}
where the kinetic energy is
\begin{equation}
T = \frac{1}{2} (I_b + m_b l_b^2 + I_w + m_w l_w^2) \dot{\theta}^2 
    + I_w \dot{\theta} \dot{\phi} 
    + \frac{1}{2} I_w \dot{\phi}^2,
\end{equation}
and the potential energy due to gravity is
\begin{equation}
V = - M g \cos\theta, \quad M = m_b l_b + m_w l_w.
\end{equation}

Thus, the total energy can be written explicitly as
\begin{equation}
E = \frac{1}{2} (I_b + m_b l_b^2 + I_w + m_w l_w^2) \dot{\theta}^2 
    + I_w \dot{\theta} \dot{\phi} 
    + \frac{1}{2} I_w \dot{\phi}^2 
    - M g \cos\theta.
\end{equation}

Taking the time derivative of the total energy, we obtain
\begin{equation}
\dot{E} = (I_b + m_b l_b^2 + I_w + m_w l_w^2) \dot{\theta} \ddot{\theta} 
         + I_w (\dot{\theta} \ddot{\phi} + \dot{\phi} \ddot{\theta}) 
         + I_w \dot{\phi} \ddot{\phi} 
         + M g \sin\theta \, \dot{\theta}.
\end{equation}

Substituting the Euler-Lagrange equations of motion for the system from Eqn.(\ref{eqn_eomb}) and Eqn.(\ref{eqn_eomw}).
\begin{equation}
\dot{E} = \dot{\phi} \, \tau_m.
\label{eq:passiv}
\end{equation}

We have also considered the absence of friction torque $\tau_f$ and $\tau$ is equal to $\tau_m$ for the following sections.
This expression shows that the rate of change of total energy is equal to the power supplied by the motor torque, and therefore the system satisfies the \textbf{passivity property}.


\subsection{Energy Based Swing-Up Controller}
...
\begin{equation}
V = \frac{1}{2} k_E E^2 + \frac{1}{2} k_v \dot{\phi}^2 + \frac{1}{2} k_p \phi^2
\label{eq:lyapunov}
\end{equation}

Taking the time derivative of $V$,
\begin{equation}
\dot{V} = k_E E \dot{E} + k_v \dot{\phi} \ddot{\phi} + k_p \phi \dot{\phi}
\label{eq:vdot}
\end{equation}

Using the relation from passivity property in eqn. (\ref{eq:passiv}) we obtain
\begin{equation}
\dot{V} = k_E E \tau \dot{\phi}
+ k_v \dot{\phi} \ddot{\phi}
+ k_p \phi \dot{\phi}
\label{eq:vdot_sub}
\end{equation}
...
\dot{\phi}
\left(
k_E E \tau + k_v \ddot{\phi} + k_p \phi
\right)
\label{eq:vdot_factored}
\end{equation}

Using the system dynamics given by Eqn.(\ref{eqn_eomb}) and Eqn.(\ref{eqn_eomw}), we get
...
\Bigg[
k_E E \tau
+ k_v \tau \left( \frac{I + I_w}{I I_w} \right)
- k_v \frac{m g l}{I} \sin\phi
+ k_p \phi
\Bigg]
...
Let,
\begin{equation}
k_1 &= k_v \frac{I + I_w}{I I_w},
\qquad
k_2 &= k_v \frac{m g l}{I}
\label{eq:lumped_constants}
\end{equation}
...
+ k_p \phi
\right]
\label{eq:vdot_rearranged}
\end{equation}
\subsection*{Control Design}

Assume the control input satisfies
\begin{equation}
\tau (k_E E + k_1)
- k_2 \sin\phi
+ k_p \phi
...
\begin{equation}
\dot{V} = -k_d \dot{\phi}^2
\label{eq:vdot_negative}
\end{equation}

which is negative semi-definite.

Solving \eqref{eq:assumption} for $\tau$, the control law is obtained as
\begin{equation}
\tau =
\frac{
- k_d \dot{\phi}
...
For the case $\dot{V} = 0$, LaSalle's invariance principle is invoked to analyze the asymptotic behavior of the system.


\subsection*{LaSalle's Invariance Principle Analysis}

The Lyapunov derivative satisfies
\begin{equation}
\dot{V} = -k_d \dot{\phi}^2 \leq 0 .
\end{equation}
Hence, $V(t)$ is non-increasing and all closed-loop trajectories are bounded.

Define a compact, positively invariant set $\Omega$ such that all trajectories evolve in $\Omega$. Let
\begin{equation}
\mathcal{I} = \{ x \in \Omega \mid \dot{V}(x) = 0 \}.
\end{equation}

From $\dot{V} = -k_d \dot{\phi}^2$, it follows that
\[
\mathcal{I} = \{ x \in \Omega \mid \dot{\phi} = 0 \}.
\]

Let $\mathcal{M}$ denote the largest invariant set contained in $\mathcal{I}$.  
By LaSalle's invariance principle, every closed-loop trajectory starting in $\Omega$ approaches $\mathcal{M}$ as $t \to \infty$.

In the set $\mathcal{I}$, $\dot{\phi} = 0$ and therefore $\phi$ is constant. Since
\[
\dot{E} = \tau \dot{\phi},
\]
the energy $E$ is also constant on $\mathcal{I}$.

Consequently, all system trajectories converge to the largest invariant set $\mathcal{M} \subseteq \{ \dot{\phi} = 0 \}$.


\subsubsection*{The stabilizer control law}


For our stabilizer control law, first term compensates gravity, second damps the robot velocity, third compensates friction, and the last damps the wheel velocity. If we are near the equilibrium angle, then gravity compensation can be considered linear with the approximation $\sin\theta\approx\theta$ which linearizes the control law to be
\begin{equation}
...
where, $k_{p}=2Mg$, $k_{d}=k_{2}$, and $k_{w}=-k_{1}(I+I_w)/(II_{w})$.

\subsection{Another Swing-up controller}
...
\subsubsection*{Behavior Cloning Controller}
A behavior cloning controller just learns a mapping from states to expert actions by treating control as a supervised learning problem.\\
For our system, the network maps the current state of the system to a control action by passing the input through two hidden layers of 64 neurons each, 
using ReLU activations, and outputs a single control value which is based on supervised loss.

\subsubsection*{DAgger Algorithm}
DAgger (Dataset Aggregation) \cite{dag} tackles the core flaw of basic behavioral cloning: the policy only sees expert states,
 so it falls apart when its own mistakes push it into unseen regions. DAgger fixes this by repeatedly rolling out the learned policy, 
 collecting the states it actually visits, and asking the expert to label those states with the correct actions.
 Each iteration augments the training set with these new expert-labeled samples, gradually covering the full distribution
  induced by the learned policy rather than the narrow distribution of expert demonstrations.

\subsubsection*{Q-loss}
This is an approach to imitate NMPC as introduced in the paper \cite{Qloss} in which instead of supervised learning from control inputs , the NMPC problem
is solved for each state/ input to the network while training while introducing a constraint that fixes the first control to the network predicted control 
and the cost function value is used as the loss and the lagrange multiplier corresponding to the constraint that fixes first control to the network predicted 
control which happens to be equal to gradient of the cost function with respect to this network predicted first control input is used as the gradient for 
