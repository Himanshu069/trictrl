...
Q_energy = 100.0       # weight on squared energy difference (tune)
theta_sigma = 0.15     # width for switching to stabilization (radians). smaller => stabilizer only active very near upright
Q_theta_local = 200.0 # stabilizing theta weight (used only when near upright by w_stab)
Q_thetadot_local = 5.0
Q_phidot = 0.1
R = 0.01

# Precompute symbols you will use inside loop (these are CasADi SX ops so OK to reuse)
# Note: M, g_grav, I are Python floats from your file and available here.
...
for k in range(N):
    ...
    E = 0.5 * I * X[1,k]**2 + 0.5 * Iw * (X[1,k]+X[3,k])**2 -M *g_grav* ca.cos(X[0,k])

    # squared energy error (we want E -> 0)
    energy_cost = Q_energy * ((E-Eref)**2)

    # stabilization weight: close to 1 near upright, decays away from upright
    w_stab = ca.exp(- (th/theta_sigma)**2)

    # stabilizing quadratic (activated near upright)
    stab_cost = w_stab * (Q_theta_local * th**2 + Q_thetadot_local * th_dot**2)

    # small penalty on wheel speed and control effort
    wheel_cost = Q_phidot * ph_dot**2
    control_cost = R * uk**2

    # stage cost
    obj += energy_cost + stab_cost + wheel_cost + control_cost
...
thdotN = X[1, N]
phdotN = X[3, N]
E_N = 0.5 * I * X[1,N]**2 + 0.5 * Iw * (X[1,N]+X[3,N])**2 -M *g_grav* ca.cos(X[0,N])
wN = ca.exp(- (thN/theta_sigma)**2)
obj += Q_energy * ((E_N-Eref)**2) + wN * (Q_theta_local * thN**2 + Q_thetadot_local * thdotN**2) + Q_phidot * phdotN**2 
...
x_current = np.array([0.7455, 0.0, 0.0, 0.0])
